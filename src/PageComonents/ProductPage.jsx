...
function ProductPage() {
  ...
  return (
    <div className="bg-white min-h-screen">
      <div className="w-full bg-emerald-50/50 py-12 px-10 border-b border-emerald-100">
      ...
      <div className="max-w-7xl mx-auto px-10 py-6 flex flex-col sm:flex-row justify-between items-center border-b border-gray-100 gap-4">
        ...
        <>
          ...
          <div className="flex flex-col items-center gap-10 pb-1">
            ...
            <div className="flex items-center gap-3">
                ...
                className="w-10 h-10 flex items-center justify-center border border-emerald-900 rounded-full disabled:opacity-20 hover:bg-emerald-900 hover:text-white transition-all"
                >←</button>
                {[...Array(totalPages)].map((_, i) => (
                <button
                    key={i}
                    onClick={() => { setPage(i + 1); window.scrollTo({ top: 450, behavior: "smooth" }); }}
                    className={`w-10 h-10 rounded-full text-xs font-bold ${page === i + 1 ? "bg-emerald-900 text-white" : "text-emerald-900 hover:bg-emerald-50"}`}
                >{i + 1}</button>
                ))}
                <button
