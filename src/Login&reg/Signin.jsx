...
function SignIn() {
  ...
  const handleGoogleLogin = async (credentialResponse) => {
    try {
      ...
      if (userData.role.toLowerCase() === "admin") navigate("/admin/admindash");
      else navigate("/");
    } catch (err) {
      ...
      <div className="w-full lg:w-1/2 flex flex-col p-6 md:p-10 relative">
        ...
        <div className="max-w-sm w-full mx-auto flex-1 flex flex-col justify-center">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <button disabled={loading} className="w-full bg-[#1a3a32] text-white py-3.5 rounded-sm text-[11px] uppercase tracking-[0.3em] font-bold hover:bg-[#122b25] transition-all shadow-xl mt-4">
          ...
          <div className="flex justify-center">
            <GoogleLogin onSuccess={handleGoogleLogin} onError={() => toast.error("Login Failed")} theme="outline" shape="rectangular" width="320" />
          </div>
